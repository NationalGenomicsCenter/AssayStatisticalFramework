...
Out_GenBank <- "nemouridae_sequences.fas" # Outputs sequence FASTA file
Out_table <- "nemouridae_records.csv" # Outputs sequence metadata (CSV)
...
##################################################################################################
### Optional code to pull manually selected sequences
##################################################################################################
# ### Load packages and set up data frames
# library(seqinr)
# 
...
# All_seqs <- "ASA2_BLAST_hits_5seqs.fas"
# Wanted_seqs <- "ASA2_BLAST_accns.txt" # List of accession numbers/titles (as in CSV file; one per row)
# Wanted_seqs_out <- "ASA2_BLAST_selected_seqs.fas"
# 
# ### Read FASTA file for all sequences
# All_seqs <- read.fasta(All_seqs, as.string=T, set.attributes=T, whole.header=T, forceDNAtolower=F)
# 
# ### Read TXT file for wanted sequences (no "#" as comment character) and replace ">" for "_"
# Wanted_seqs <- read.table(Wanted_seqs, sep="\n", comment.char="")$V1
# Wanted_seqs <- gsub(">", "_", Wanted_seqs)
# 
# ### Retrieve FASTA file for wanted sequences
# Wanted_seqs <- All_seqs[names(All_seqs) %in% Wanted_seqs]
# 
# ### Write FASTA file for wanted sequences
# write.fasta(Wanted_seqs, names=attr(Wanted_seqs, "name"), file.out=Wanted_seqs_out)
# print("Finished!")
